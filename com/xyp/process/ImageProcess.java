...
import com.xyp.process.Threader;
import com.xyp.util.Random;

/**
 * 用了两种写法，第一种是把map和reduce写在一个类里面，第二种写法是把它们分开写
 * @author 大象
 */
public class ImageProcess extends Configured implements Tool {
	 ...
	 * 
	 */
	public static class ImageProcessMapper extends
			Mapper<LongWritable, BytesWritable, BooleanWritable, Text> {
		...
		public void setup(Context context) {
			...
			bf = new BundleFile(new Path(temp + Random.random() + "."
					+ conf.get("process.outtype")), conf);
...
//				him = runNormal(him);
		...
		public HImage runNormal(HImage him) {
			try {
				GenericAlgorithm pi = (GenericAlgorithm) Class.forName(
						conf.get("process.class")).newInstance();
				pi.setHImage(him);
		...
		public HImage runThreaded(HImage him) throws InterruptedException {
			...
			for (int i = 0; i < split.length; i++) {
				thr[i] = new Thread(new Threader(split[i],
						conf.get("process.class")));
				thr[i].start();
			}
		...
		public HImage[] splitImages(HImage img) {
			int rows = 8; // 把一张图像分成多少行，多少列的小chunk
			int cols = 1;
	...
	public static class ImageProcessReducer extends Reducer<BooleanWritable, Text, BooleanWritable, Text> {
		...
		public void setup(Context jc) throws IOException {
			...
			bf = new BundleFile(new Path(conf.get("process.output") + "."
					+ conf.get("process.outtype")), conf);
			bw = bf.getBundleWriter();
		}
	...
	public static void removeDir(String path, Configuration conf)
			throws IOException {
		Path output_path = new Path(path);
		FileSystem fs = FileSystem.get(conf);
		if (!fs.exists(output_path)) {
